...
def fit_patterns(
    ...
    if verbosity > 0:
        print(f"iter {0}, ELBO: {elbo_1:0.2e}")
    ...
    for i in range(maxiter):
        ...
        if verbosity > 0 and (i % printerval == 0):
            print(f"iter {i + 1}, ELBO: {elbo_1:0.2e}, delta_ELBO: {delta_elbo:0.2e}")
        ...
        if delta_elbo < delta_elbo_threshold:
            if verbosity > 0:
                print(f"Converged on iter {i + 1}")

            break

    else:
        print(f"Warning! ELBO dit not converge after {i + 1} iters!")
...
def spatial_patterns(
    ...
    **kwargs are passed on to the function fit_patterns()
    ...
    cres = pd.DataFrame(
        {"g": DE_mll_results["g"], "pattern": r.argmax(1), "membership": r.max(1)}
    )
